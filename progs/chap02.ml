...
let rec loop (c:RTCCanvas.t) clr env ((pos:RTCTuple.t), _ as proj) =
  if pos.y <= 0. then c
...
let run () =
  ...
  let ppm  = RTCCanvas.to_ppm (loop c clr (grav, wind) (pos, vel)) in
  let f    = open_out "02-trajectory.ppm" in
  output_string f ppm;
  Printf.printf "wrote `02-trajectory.ppm'\n"
