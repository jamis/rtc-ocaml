let draw_sphere sphere ray_z wall_z canvas_size =
  ...
  let draw_hit canvas r x y = function
    ...
    | Some (hit : RTCShape.t RTCIntersection.t) ->
      ...
      let identity x = x in
      let color = RTCMaterial.lighting (RTCShape.material hit.shape) identity light point eye normal false in
...
let run () =
  ...
  let f = open_out "06-sphere-shaded.ppm" in
  output_string f ppm;
  Printf.printf "wrote `06-sphere-shaded.ppm'\n"
