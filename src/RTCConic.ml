...
let intersect_caps shape min max closed (ray:RTCRay.t) radius_at xs =
  ...
  if (not closed) || ((abs_float ray.direction.y) < RTCConst.epsilon) then
    ...
    let xs' =
      ...
      if check_cap t (radius_at min) then (RTCIntersection.build t shape) :: xs else xs
    in
    let t = (max -. ray.origin.y) /. ray.direction.y in
    if check_cap t (radius_at max) then (RTCIntersection.build t shape) :: xs' else xs'

let intersect (shape:RTCShape.t) (r:RTCRay.t) afn bfn cfn radius_at =
  ...
  in
  RTCIntersection.list (intersect_caps shape minimum maximum closed r radius_at xs)
